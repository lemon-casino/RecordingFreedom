...
状态：preview 发布链路已能产出可下载验收包。`v0.1.0-preview.14` 已作为 GitHub prerelease 发布，Release Gate、Windows x64、macOS arm64、Linux x64 和 Publish GitHub Release 均通过；产物包含 Windows portable zip、macOS arm64 raw preview binary、Linux x64 raw preview binary 和 SHA256SUMS。该版本把真实麦克风设备枚举/电平监听、真实 WASAPI endpoint 选择、macOS CoreAudio 麦克风代码路径、Windows GUI subsystem、FFmpeg 子进程命令窗口隐藏、录制态配置锁定、区域录制持久边框、胶囊透明背景、屏幕编号标识、区域编辑 overlay、录制态透明 overlay 边框和 CoreAudio annotation 清理纳入验收包。正式签名安装包、公证、Linux 包格式和 clean-machine 验收仍未完成。
