...
- `system-audio.wav` / `microphone.wav` 保留为过渡、诊断、恢复或平台 fallback sidecar；当平台 writer 已经把对应音轨 mux 进 `screen.mp4` 后，manifest 和 ready 门禁必须能区分 muxed track 与 sidecar track，不能强迫重复写两份音频。
...
- fallback sidecar 只用于 smoke、恢复、平台限制和诊断场景。manifest 需要能表达每条音频是 `muxed` 还是 `sidecar`，ready 门禁据此检查 `screen.mp4` 内音轨或包内 sidecar，避免长期重复写盘。
...
当前 macOS build-tag 已有 CoreGraphics source enumeration，并已把 `screen:display-<CGDirectDisplayID>`、`window:<CGWindowID>` 和 `application:<pid>` 映射到 ScreenCaptureKit capture target。`application:<pid>` 首版选择该 PID 当前最大可见窗口；后续可升级为多窗口组合或平台 app capture target。
...
- ready 门禁会拒绝只有 44-byte WAV header 的音频 sidecar；启用 sidecar 的音频流必须写入至少一个样本，避免无 packet 的系统声音被误标为真实音频。后续 muxed audio track 门禁要改为 probe `screen.mp4` 内的音轨，而不是要求重复 sidecar。
...
后续应增加 audio-only 录制模式，供用户只录系统声音、麦克风或麦克风 + RNNoise。audio-only 不应该伪装成屏幕录制，也不应该创建假的 `screen.mp4`：

- 包类型仍使用 `.rfrec`，路径仍位于 `<DataRoot>/data/video/`，但 manifest 需要新增可区分的 recording kind 或 source type。
- 默认输出应使用可流式写入、可恢复的音频容器，例如 `audio.m4a` 或 `audio.wav`；正式发布前再按平台能力选择 AAC/M4A 或 WAV fallback。
