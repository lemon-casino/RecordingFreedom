...
当前 preview release 必须在 release notes 中明确：macOS ScreenCaptureKit display capture 已接入代码路径但仍需真机 smoke 验收；ScreenCaptureKit window/program capture、Windows WGC、Linux PipeWire、完整 app recording backend 音频接入、摄像头 sidecar 和 PIP 导出仍属于后续里程碑，不能把 mock package、未验收的 ScreenCaptureKit display 路径或 `audio-smoke` 说成完整真实录制。
