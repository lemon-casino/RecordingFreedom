...
- `v0.1.15` 已作为本轮智能区域识别验收版本发布，Release workflow 已通过：<https://github.com/lemon-casino/RecordingFreedom/actions/runs/28736682322>。
- 对应提交为 `a740d3fe33d7b5545c1b44c7864ce7384f79040c`。
- 用户下载并测试后反馈“已经没有问题了”，本轮 snow-shot 风格动态区域识别按已验收处理。
- 已验收范围包括：截图区域识别、滚动截图目标区域识别、录制中/截图画板区域识别、自定义区域录制框选、hover 动态识别、候选层级滚轮切换、右键取消以及 pending hover assist 阶段取消。
- 后续如果再次出现识别不准、右键取消失效、候选层级丢失、多屏坐标偏移或画板区域回退，应作为 `v0.1.15` 基线之后的新回归处理，不能把已验收行为删掉或换回临时方案。
