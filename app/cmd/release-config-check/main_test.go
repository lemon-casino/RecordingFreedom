...
func workflowFixture(name string) string {
	...
	builder.WriteString("cgo_enabled: \"1\"\n")
	builder.WriteString("rnnoise_native: \"true\"\n")
	builder.WriteString("cgo_enabled: \"0\"\n")
	builder.WriteString("rnnoise_native: \"false\"\n")
	...
	if strings.Contains(name, "release.yml") {
		builder.WriteString("RNNoise native DSP is compiled into Windows x64, macOS x64/arm64, and Linux x64/arm64 artifacts\n")
		builder.WriteString("Windows ARM64 is published as a native ARM64 recorder package without rnnoise_native\n")
