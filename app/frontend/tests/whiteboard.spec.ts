...
test.describe('whiteboard capsule localization and themes', () => {
  for (const scenario of cases) {
    test(`${scenario.name} renders the complete capsule`, async ({page}) => {
      ...
      await expect(page.locator('.whiteboard-tools button')).toHaveCount(12)
      await expect(page.getByRole('button', {name: scenario.locale === 'zh-CN' ? '菱形' : 'Diamond'})).toBeVisible()
      await expect(page.getByRole('button', {name: scenario.locale === 'zh-CN' ? '插入图片' : 'Insert image'})).toBeVisible()
      await expect(page.getByRole('button', {name: scenario.penLabel})).toHaveClass(/selected/)
      const greenSwatch = page.locator('.whiteboard-style-group button[title="#22c55e"]')
      await greenSwatch.click()
      await expect.poll(async () => greenSwatch.evaluate((element) => getComputedStyle(element).backgroundColor)).toBe('rgb(34, 197, 94)')
      ...
      await expect(page.locator('.whiteboard-canvas .excalidraw .layer-ui__wrapper')).toBeHidden()
